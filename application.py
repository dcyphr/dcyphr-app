...
def index():
    if request.method == "GET":
        ...
        results = db.execute("SELECT summary.doi, summary.summary, username, article, users.id FROM summary JOIN users ON summary.user_id = users.id WHERE (article LIKE :search OR summary.doi LIKE :search OR username LIKE :search OR summary.summary LIKE :search OR summary.citation LIKE :search) AND summary.done = CAST(1 AS BIT)", search="%" + search + "%")
        soup = []
        links = []
