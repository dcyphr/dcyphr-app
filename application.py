    ...
    if request.method == "GET":
        ...
        # Handles comments
            ...
            if not reply:
                ...
                comment_id = db.execute("SELECT id FROM comments WHERE doi=:doi AND comment=:comment ORDER BY id DESC LIMIT 1", doi=doi, comment=comment)[0]["id"]
                db.execute("UPDATE comments SET comment_id=:comment_id WHERE doi=:doi AND comment=:comment ORDER BY id DESC LIMIT 1", comment_id=comment_id, doi=doi, comment=comment)
            # Reply
            else:
...
def approvals(approval_id):
    if request.method == "GET":
        ...
        approve = request.form.get("approve")
        doi = db.execute("SELECT doi FROM summary WHERE id=:approval_id", approval_id=approval_id)[0]['doi']
        if not approve:
            db.execute("UPDATE summary SET summary = '', user_id=NULL, done=CAST(0 AS BIT) WHERE doi=:doi", doi=doi)
        else:
            user_id = db.execute("SELECT user_id FROM summary WHERE doi=:doi", doi=doi)[0]["user_id"]
            ...
            db.execute("UPDATE summary SET summary = :summary, approved=1 WHERE doi=:doi", summary=summary, doi=doi)
