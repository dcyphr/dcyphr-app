...
def requesting():
    if request.method == "GET":
        ...
        if len(db.execute("SELECT doi FROM summary WHERE doi=:doi", doi=doi)) > 0:
            ...
            db.execute("INSERT INTO summary (requests, article, doi, done, link, citation, likes) VALUES (1, :article, :doi, CAST(0 AS BIT), :link, :citation, 0);",
...
def index():
    if request.method == "GET":
        ...
        results = db.execute("SELECT summary.doi, summary.summary, username, article, users.id FROM summary JOIN users ON summary.user_id = users.id WHERE (article LIKE :search OR summary.doi LIKE :search OR username LIKE :search OR summary.summary LIKE :search OR summary.citation LIKE :search) AND summary.done = CAST(1 AS BIT) AND summary.approved = 1", search="%" + search + "%")
        soup = []
        links = []
