...
engine = create_engine('postgres://hwicvwhg:4zzgStNJkiEy3hC3gtFHrdlyLFR_vQUN@rajje.db.elephantsql.com:5432/hwicvwhg?sslmode=require') # database engine object from SQLAlchemy that manages connections to the database
...
def browse():
    ...
    destination = db.execute("SELECT doi FROM summary WHERE summary.done = CAST(1 AS BIT)").fetchall()
    ...
    summaries = db.execute(
        "SELECT article, username, users.id FROM summary JOIN users ON summary.user_id = users.id WHERE summary.done = CAST(1 AS BIT);").fetchall()
