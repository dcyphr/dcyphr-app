...
#send confirmation email
            ...
            message = Mail(
                ...
                html_content='<h2 style="font-family: Georgia">Welcome to <span style="color: #017bff">dcyphr</span>!<br><a href={}><button class="btn btn-outline-primary border20">Confirm account</button></a>'.format(confirm_url))
            try:
...
def unconfirmed(user_id):
    if request.method == "GET":
        ...
        email = db.execute("SELECT email FROM users WHERE id=:user_id", user_id=user_id)[0]['email']
        token = generate_confirmation_token(email)
        confirm_url = url_for('confirm_email', token=token, _external=True)
        message = Mail(
            from_email='team@dcyphr.org',
            to_emails=email,
            subject='Confirm Your dcyphr Account',
            html_content='<a href={}>Click to confirm your dcyphr account.</a>'.format(confirm_url))
        try:
            sg = SendGridAPIClient('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')
            response = sg.send(message)
            print(response.status_code)
